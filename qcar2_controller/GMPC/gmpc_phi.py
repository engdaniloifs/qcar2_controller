...
class GeometricMPC_phi:
    ...
    def set_control_bound(self, v_min = -100, v_max= 100, phi_min = -100, phi_max= 100):
        self.v_min = v_min
        self.v_max = v_max
    ...
    def solve(self, current_state):
        ...
        # x_next = A * x + B * u + h
        for i in range(N):
            u_d = self.ref_control[:, i]  # desired control input
            u_twist = self.vel_cmd_to_local_twist(u_d)

            A = -SE2Tangent(u_twist).smallAdj()
            B = np.eye(self.nTwist)
            h = -self.convert_to_dynamics(u_d)
            twist_inputs = self.ackerman_input_to_local_twist(u_var[:, i], u_d)
        ...
        opti.subject_to(u_var[1, :] >= self.phi_min)
        opti.subject_to(u_var[1, :] <= self.phi_max)
    ...
    def ackerman_input_to_local_twist(self, ackermann_input,desired_vel_cmd_input):
        v = ackermann_input[0]
        phi = ackermann_input[1]
        v_d, w_d, phi_d = desired_vel_cmd_input[0], desired_vel_cmd_input[1], desired_vel_cmd_input[2]

        w = (np.tan(phi_d)/0.256) * (v - v_d) + (v_d/(0.256*np.cos(phi_d)**2))*(phi - phi_d)
        
        return ca.vertcat(v,0, w)

    def convert_to_dynamics(self, vel_cmd):
        v = vel_cmd[0]
        return ca.vertcat(v, 0, 0)
    
    def vel_cmd_to_ackerman_input(self, vel_cmd):
        v, w, phi = vel_cmd[0], vel_cmd[1], vel_cmd[2]
        
        return ca.vertcat(v, phi)
